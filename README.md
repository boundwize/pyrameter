...
Use `usesClass()` for a specific class, `usesNamespace()` for a namespace prefix, and `usesFunction()` for a function call that should classify a test as a heavier kind. Class, namespace, and function matching is case-insensitive and accepts names with or without a leading backslash.
